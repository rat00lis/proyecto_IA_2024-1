...
class DQN_Agent():
    def __init__(self):
        ...
        if DEVICE == 'cuda':
            self.policy_net.load_state_dict(torch.load('./DQN/training_DQN/policy_w.pth'))
        else:
            self.policy_net.load_state_dict(torch.load('./DQN/training_DQN/policy_w.pth', map_location=torch.device('cpu')))
